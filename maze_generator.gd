...
# Meta-labirinto (labirinto 10x10 di celle)
...
func generate_meta_maze():
	## Genera il meta-labirinto 10x10 usando Eller
	...
	for row in range(meta_maze_size):
		...
		if row < meta_maze_size - 1:
			...
			for set_id in set_groups.keys():
				...
				for col in columns:
					if rng.randf() > 0.5 or not connections_made:
						# Connetti verticalmente
						add_meta_connection(col, row, col, row + 1)
						new_sets[col] = set_id
...
func add_meta_connection(x1: int, y1: int, x2: int, y2: int):
	## Aggiunge una connessione bidirezionale nel meta-labirinto
	var key1 = Vector2i(x1, y1)
	var key2 = Vector2i(x2, y2)
	
	if not meta_maze_connections.has(key1):
		meta_maze_connections[key1] = []
	if not meta_maze_connections.has(key2):
		meta_maze_connections[key2] = []
	
	meta_maze_connections[key1].append(key2)
	meta_maze_connections[key2].append(key1)

func select_random_starting_cell():
	## Seleziona casualmente una cella di partenza
	current_cell_x = rng.randi_range(0, meta_maze_size - 1)
	current_cell_y = rng.randi_range(0, meta_maze_size - 1)
	print("Cella iniziale: (", current_cell_x, ", ", current_cell_y, ")")
...
func generate_current_cell_maze():
	...
	# Crea un seed unico per questa cella
	var cell_seed = seed_value + current_cell_x * 1000 + current_cell_y
